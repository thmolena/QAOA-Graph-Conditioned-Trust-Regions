...
def _implementation_files() -> list[Path]:
    ...
    portfolio_only = {
        ...
        "protocol.py", "provenance.py", "risk_control.py",
    }
...
def _implementation_fingerprint() -> str:
    ...
    for path in _implementation_files():
        ...
        digest.update(b"\0")
        digest.update(artifact_source_bytes(path))
        digest.update(b"\0")
    return digest.hexdigest()
...
def validate_manifest(manuscript_dir: Path, figures_dir: Path | None = None) -> dict:
    ...
    core_fingerprint = _implementation_fingerprint()
    recorded_fingerprint = manifest.get("replot_implementation_sha256")
    compatible_fingerprints = compatible_manifest_fingerprints(
        core_fingerprint)
    if recorded_fingerprint not in compatible_fingerprints:
        errors.append(
            "manifest replot_implementation_sha256: expected one of "
            f"{sorted(compatible_fingerprints)!r}, "
            f"got {recorded_fingerprint!r}")
...
def _default_manuscript_dir() -> Path:
    ...
    the working directory or package. In a repository checkout the package lives at
    ...
    if (cwd / "main.tex").is_file():
        return cwd
    if (cwd / "manuscript" / "main.tex").is_file():
        return cwd / "manuscript"
